...
export default class Experience {
  constructor(_canvas) {
    ...
    //Generar obstaculos
    // this._startObstacleWaves() // ❌ DESACTIVADO - No generar obstáculos automáticos
  ...
  destroy() {
    // Limpiar timer de obstáculos si existe
    if (this.obstacleWaveTimeout) {
      clearTimeout(this.obstacleWaveTimeout)
      this.obstacleWaveTimeout = null
    }
  ...
  startGame() {
    ...
    if (this.world) {
      ...
      // 🎵 Iniciar música ambiente al comenzar el juego
      if (this.world.ambientSound) {
        try {
          this.world.ambientSound.play()
          console.log('🎵 Música ambiente iniciada: /sounds/ambiente.mp3')
        } catch (error) {
          console.error('❌ Error al iniciar música ambiente:', error)
        }
      }
    }
  ...
  resetGame() {
    ...
    // Detener todos los enemigos antes de reiniciar
    if (this.world && this.world.stopAllEnemies) {
      this.world.stopAllEnemies()
    }
    
    // 🎵 Detener música ambiente al reiniciar
    if (this.world && this.world.ambientSound) {
      this.world.ambientSound.stop()
      console.log('🔇 Música ambiente detenida')
    }
